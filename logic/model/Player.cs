...
public class Player : IEquatable<Player>
{
    ...
    /// <summary>
    /// True when this player's UUID is an offline-mode (version-3, name-based) UUID -
    /// generated by the server from the username rather than assigned by Mojang.
    /// Detected from the UUID version nibble (char index 12 of the dash-stripped UUID).
    /// </summary>
    [JsonIgnore]
    public bool IsOfflineMode
    {
        get
        {
            ...
            return u.Length >= 13 && u[12] == '3';
        }
    }

    /// <summary>
    /// Display-only name. Offline-mode players are prefixed "OM-" so they are obvious in
    /// the UI. The stored <see cref="Name"/> is never modified - command routing
    /// (op/kick/ban/whitelist) continues to use the real name.
    /// </summary>
    [JsonIgnore]
    public string DisplayName => IsOfflineMode ? "OM-" + Name : Name;
